import XCTest
@testable import Potshelf

@MainActor
final class PotshelfTests: XCTestCase {
    var store: Store!
    ...
    func testAddEntryIncreasesCount() {
        ...
        store.add(PlantEntry(title: "Test", detail1: "A", detail2: "B", note: "", date: Date()))
        XCTAssertEqual(store.entries.count, before + 1)
    }

    func testDeleteEntryDecreasesCount() {
        store.add(PlantEntry(title: "ToDelete", detail1: "A", detail2: "B", note: "", date: Date()))
        let before = store.entries.count
    ...
    func testCannotAddMoreWhenAtLimitAndNotPro() {
        while store.entries.count < Store.freeLimit {
            store.add(PlantEntry(title: "Filler", detail1: "A", detail2: "B", note: "", date: Date()))
        }
    ...
    func testUpdateEntryChangesTitle() {
        store.add(PlantEntry(title: "Original", detail1: "A", detail2: "B", note: "", date: Date()))
